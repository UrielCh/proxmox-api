...
export class ProxmoxEngine implements ApiRequestable {
    ...
    public async doRequest(
        ...
        if (contentType === "application/json;charset=UTF-8") {
            ...
            // should never append
            throw Error(
                `${httpMethod} ${requestUrl} unexpected contentType "${contentType}" status Line:${res.statusCode} ${res.text}`
            );
            // data.data = req.text();
        }
        ...
        switch (res.statusCode) {
            case 400:
                throw Error(
                    `${httpMethod} ${requestUrl} return Error ${
                        res.statusCode
            ...
            default:
                throw Error(
                    `${httpMethod} ${requestUrl} connetion failed with ${
                        res.statusCode
                    } ${res.statusText} return: ${JSON.stringify(data)}`
                );
        }
    }
    ...
    public async getTicket(): Promise<{
        ...
        try {
            ...
            if (req.statusCode !== 200) {
                throw Error(
                    `login failed with ${req.statusCode}: ${req.statusText}`
                );
            }
